...
var authEndpoint = 'https://login.microsoftonline.com/common/oauth2/v2.0/authorize?';
  ...
  var appId = 'e84e606a-4ddf-406f-b23e-a1037f9d6801';
  var scopes = 'openid profile User.Read Mail.Read Calendars.ReadWrite';
  ...
  async function renderCalendar() {
    ...
    await new Promise(resolve => {
      getUserEvents(async function(events, error){
      if (error) {
        renderError('getUserEvents failed', error);
       // $('event-list').append("No events found");
      } else {
          ...
          for(let i in events) {
            ...
            let d = new Date(events[i].start.dateTime);
            d.setHours(d.getHours() - 7);
            events[i].start.dateTime = d;

            d = new Date(events[i].end.dateTime);
            d.setHours(d.getHours() - 7);
            events[i].end.dateTime = d;
          }
  ...
  function getUserEvents(callback) {
    getAccessToken(function(accessToken) {
      if (accessToken) {
        ...
        client
          .api('/me/calendar/calendarView?startDateTime=' + (startDate.value + 'T00:00:00') + '&endDateTime=' + (endDate.value + 'T23:59:59'))
          .select('subject,start,end,createdDateTime')
          //.filter("start/dateTime ge '" + startDate.value + "' and start/dateTime le '" + endDate.value +"'")
          .top(200)
  ...
  async function get_GlobalEventData(){
    await new Promise(async resolve => {
      await renderCalendar().then(() => resolve());
    });
...
function createEvent_Outlook(startDate, description){
  ...
  getAccessToken(async function(accessToken) {
    ...
    dT = new Date(startDate.getTime() + 10800000);

    const event = {
      subject: description,
      body: {
        contentType: "HTML",
        content: "Created from Paul Gurman's app"
      },
      start: {
        dateTime: dT,
        timeZone: dT.toString().slice(35, dT.toString().length - 1)
      },
      end: {
    ...
    let res = await client.api('/me/events')
      .post(event)
      .then(() => { renderCalendar(); console.log("Outlook complete");});
